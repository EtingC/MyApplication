...
public class NumberPickerView extends View {
    ...
    /**
     * 为了更好的复用写这个方法来改变显示的行数
     * @param a
     */
    public void setShowCount(int a){
        if (a!=0){
            mShowCount=a;
            postInvalidate();
        }
    ...
    private void updateDividerAttr(){
        ...
        dividerY0 = mDividerIndex0 * mViewHeight /(float) mShowCount;
        dividerY1 = mDividerIndex1 * mViewHeight / (float) mShowCount;
        ...
        for(int i = 0; i < mShowCount + 1; i++){
            ...
            if(0 <= index && index < getOneRecycleSize()){
                ...
                canvas.drawText(str.toString(), mViewCenterX,
                        y + mItemHeight / 2.0f + textSizeCenterYOffset, mPaintText);
    ...
    private void drawHint(Canvas canvas){
        ...
        canvas.drawText(mHintText,
                mViewCenterX + (mMaxWidthOfDisplayedValues + mWidthOfHintText)/2.0f + mMarginStartOfHint,
                (dividerY0 + dividerY1) / 2.0f + mTextSizeHintCenterYOffset, mPaintHint);
    }
